...
eventos = LOAD '/data/eventos.json' 
    USING JsonLoader(
      'type:chararray,
       city:chararray,
       timestamp:long,
...
eventos_formateados = FOREACH filtrados GENERATE
    type,
    city,
    ToString(FromUnixTime(timestamp/1000)) as fecha_hora,
    street,
    severity,
    reliability;

-- Guardar resultados filtrados en CSV
STORE eventos_formateados INTO '/data/eventos_filtrados.csv' USING PigStorage(',');
